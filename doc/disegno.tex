...
El diagrama de arquitectura de nuestra herramienta es el siguiente:

\todo{AB: con que generamos el diagrama de arquitectura?}
...
\todo{que listing usamos en las expresiones regulares?}
...
\item \lstinline{INSTANCE: ``instance''} (el literal ''instance")
\item \lstinline{OF: ``of''} (el literal "of")
\item \lstinline{MESSAGE: ``message''} (el literal "message")
\item \lstinline{FROM: ``from''} (el literal ''from'')
\item \lstinline{TO: ``to''} (el literal ''to'')
\item \lstinline{AT: ``at''} (el literal ''@'')
\item \lstinline{PLUS: ``+''} (el literal ''+''.)
\item \lstinline{MINUS: ``-''} (el literal ''-'')
\item \lstinline{EXCLAMATION: ``!''} (el literal ''!'')
\item \lstinline{INTERROGATION: ``?''} (el literal ''?'')
\item \lstinline{SEMICOLON: ``;''} el literal '';''.
\item \lstinline{LEFT\_BRACE: ``{''} el literal ''\{''.
\item \lstinline{RIGHT\_BRACE: ``}''} el literal ''\}''.
\end{itemize}
